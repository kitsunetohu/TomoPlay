...
public class TankHealth : MonoBehaviour , ISyncReceiverByteArray
{
    ...
    async void FixedUpdate()
    {
        if (SyncGameObject != null)
        {
            ...
            {
                await SyncGameObject.SendAsync(new TankHealthSync
                { CurrentHealth = m_CurrentHealth, ZeroHealthHappened = m_ZeroHealthHappened }, typeof(TankHealth));
            }
        }
    }
    ...
    {
        var packet = Serializer.Deserialize<TankHealthSync>(value);
        m_CurrentHealth = packet.CurrentHealth;
        m_ZeroHealthHappened = packet.ZeroHealthHappened;
    }
}

public class TankHealthSync
{
    public float CurrentHealth;
    public bool ZeroHealthHappened;
}