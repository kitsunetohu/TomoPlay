...
namespace UniP2P.Example.Tanks
{
    public class CameraControl : MonoBehaviour
    {
        ...
        public float
            m_ScreenEdgeBuffer =
                4f; // Space between the top/bottom most target and the screen edge (multiplied by aspect for left and right).
        ...
        private Vector3 FindAveragePosition()
        {
            ...
            foreach (var tank in TasksManager.Tanks)
            {
                if (tank != null)
                {
                    // Add to the average and increment the number of targets in the average.
                    average += tank.transform.position;
                    numTargets++;
                }         
            }
        ...
        private float MaxTargetDistance(Vector3 desiredPosition)
        {
            ...
            foreach (var tank in TasksManager.Tanks)
            {
                if (tank != null)
                {
                    ...
                    float targetDistance = (desiredPosition - tank.transform.position).magnitude;
